...
module.exports.songsToClusters = async function(songIdToName, songIdList, matrix, k, seed)
{
    const kmean = new scikitjs.KMeans({ nClusters: k, randomState: seed})
    kmean.fit(matrix);
...
module.exports.getCentroids = async (k, matrix, seed) =>
{
    const kmean = new scikitjs.KMeans({ nClusters: k, randomState: seed })
    kmean.fit(matrix);
